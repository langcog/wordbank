...
class ImportHelper:
        ...
        try:
            if field_type in ("study_id", "study_momed", "study_family_id", "language"):
                ...
                try:
                    ...
                    raise KeyError(
                        f"Value mapping doesn't have entry for field type { field_type } and value { value }. "
                    )
            ...
            if value in self.missing_values:
                ...
                raise KeyError(
                    f"Value mapping doesn't have entry for field type { field_type } and value { value }.  { column }"
                )
    ...
    def get_data_fields(self, cols, group, row_values):
        ...
        for column in list(group_cols.keys()):
            ...
            if field_type == "mom_ed":
                ...
                study_momed_value = self.get_field_value(
                    column, "study_momed", group, row_values
                )
                results["study_momed"] = study_momed_value
            elif field_type in ["condition", "language"]:
                if results[field_type] == None:
                    results[field_type] = []
                if self.get_field_value(column, field_type, group, row_values) != None:
                    results[field_type].append(
                        self.get_field_value(column, field_type, group, row_values)
                    )
            else:
                field_value = self.get_field_value(
                    column, field_type, group, row_values
                )
            ...
            if len(row_values) > 1:
                ...
                if column is not None and column != "":
                    ...
                    for column in columns:
                        field_mapping[group][column.lower()] = {
                            "field": field,
    ...
    def get_meta_data(self):
        if self.ftype == "xlsx" or self.ftype == "xls":
            ...
            self.get_value_mapping_row = lambda row: list(
                value_mapping_sheet.row_values(row)
            )

            field_mapping_sheet = book.sheet_by_name("fields")
            self.field_mapping_nrows = field_mapping_sheet.nrows
            self.get_field_mapping_row = lambda row: list(
                field_mapping_sheet.row_values(row)
            )
            ...
            value_mapping_file = open(
                ".".join(self.data_file.split(".")[:-1]) + "_values.csv",
                newline="",
                encoding="utf-8-sig",
            )
            ...
            field_mapping_file = open(
                ".".join(self.data_file.split(".")[:-1]) + "_fields.csv",
                newline="",
                encoding="utf-8-sig",
            )
            ...
            data_file = open(
                ".".join(self.data_file.split(".")[:-1]) + "_data.csv",
                newline="",
                encoding="utf-8-sig",
            )
    ...
    def get_row_data(self, row):
        ...
        if len(row_values) > 1:
            ...
            administration = self.get_data_fields(
                self.field_mapping, "admin", row_values
            )
            if (
                child["date_of_birth"] is not None
                and administration["date_of_test"] is not None
            ):
                computed_age = self.compute_age(
                    child["date_of_birth"], administration["date_of_test"]
                )
