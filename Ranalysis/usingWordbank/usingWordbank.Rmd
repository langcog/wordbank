...
```{r}
colors.danish <- c("rød", "gul", "grøn", "blå", "sort", "hvid") # "lille" "brun"
colors.english <- c("red",  "yellow", "green", "blue", "black", "white") # "orange" "brown"
colors.german <- c("rot", "gelb", "grün", "blau", "schwarz", "weiß") # braun orange
colors.italian <- c("rosso", "giallo", "verde", "blu", "nero", "bianco") # arancione marrone
colors.norwegian <- c ("rød", "gul", "grønn" ,"blå", "svart", "hvit") # "brun" "oransje"
colors.russian <- c("красный", "желтый", "зеленый", "синий", "черный", "белый") # "коричневый" "оранжевый"
colors.spanish <- c("rojo", "amarillo", "verde", "azul", "negro", "blanco") # "rosa" "morado"
colors.swedish <- c("röd", "gul", "grön", "blå", "svart", "vit") # orange brun

colors <- list("Danish" = colors.danish,
               "English" = colors.english,
               "German" = colors.german,
               "Italian" = colors.italian,
               "Norwegian" = colors.norwegian,
               "Russian" = colors.russian,
               "Spanish" = colors.spanish,
               "Swedish" = colors.swedish)
```
...
languages <- c("English", "Spanish", "Norwegian", "Danish",
               "Russian", "German", "Italian", "Swedish")
...
#  geom_dl(method = list(dl.trans(x=x +.2), "last.qp", cex=.8, fontfamily=font)) +
  ...
  scale_x_continuous(breaks=seq(16,30,2), limits=c(16,30),
                     name = "\nAge (months)") +
