...
vocab.admins <- admins %>%
  ...
  filter(form == "WS", age >= 16, age <= 30, !is.na(sex), sex != "")
...
colors.croatian <- c("crvena", "narančasta", "žuta", "zelena", "plava", "crn", "bijela", "smeđe")
colors.danish <- c("rød", "orange", "gul", "grøn", "blå", "sort", "hvid", "brun")
colors.english <- c("red", "orange (description)", "yellow", "green", "blue", "black", "white", "brown")
colors.german <- c("rot", "orange", "gelb", "grün", "blau", "schwarz", "weiß", "braun")
colors.italian <- c("rosso", "arancione", "giallo", "verde", "blu", "nero", "bianco", "marrone")
colors.norwegian <- c ("rød", "oransje", "gul", "grønn" ,"blå", "svart", "hvit", "brun")
colors.russian <- c("красный", "оранжевый", "желтый", "зеленый", "синий", "черный", "белый", "коричневый")
colors.spanish <- c("rojo", "", "amarillo", "verde", "azul", "negro", "blanco", "") # rosa morado
colors.swedish <- c("röd", "orange", "gul", "grön", "blå", "svart", "vit", "brun")
colors.turkish <- c("Kırmızı", "Turuncu", "Sarı", "Yeşil", "", "Siyah", "Beyaz", "Kahverengi") # Mor

colors <- list("Croatian" = colors.croatian, "Danish" = colors.danish,
               "English" = colors.english, "German" = colors.german,
               "Italian" = colors.italian, "Norwegian" = colors.norwegian,
               "Russian" = colors.russian, "Spanish" = colors.spanish,
               "Swedish" = colors.swedish, "Turkish" = colors.turkish)
```
...
ggplot(color.byage.data, aes(x=age, y=na.mean, color=color, label=color)) +
  ...
  scale_color_manual(values = c("black", "#2c7bb6", "brown", "#1a9641", "orange",
                                "#d7191c", "gray87", "gold1")) +
  scale_x_continuous(breaks = seq(16,30,2), limits = c(16,33),
                     name = "\nAge (months)") +
